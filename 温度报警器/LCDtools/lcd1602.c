...
//显示unsigned char类型的数字
void lcd1602_show_unsigned_char(u8 x,u8 y,unsigned char num)
{
	unsigned char i;
	...
	{
		content=num/count+'0';
		lcd1602_show_string(x+i,y,&content);
		num=num%count;
		count=count/10;
	}
	lcd1602_show_string(x+i,y," ");
}

//只显示两位数字（仅在显示时间时有效）
void lcd1602_show_unsigned_char_time(u8 x,u8 y,unsigned char num)
{
	unsigned char i;
...
void lcd1602_show_unsigned_char_temperature(u8 x,u8 y,float t)
{
	if(t<0)
	{
		...
		lcd1602_show_string(x,y,"+");
	lcd1602_show_unsigned_char(x+1,y,(unsigned char)((int)t));//显示整数部分
	lcd1602_show_string(x+4,y,".");//显示小数点
	lcd1602_show_unsigned_char(x+5,y,(unsigned char)(unsigned long)(t*10000)%10000);//显示
}